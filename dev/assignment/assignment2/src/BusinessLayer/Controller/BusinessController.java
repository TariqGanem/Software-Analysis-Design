...
import DataAccessLayer.Mappers.*;
import DataAccessLayer.objects.*;
...
//        }
    ...
    public void makeMinOrder() {
        for(Category category : allCategories){
            for(ItemSpecs item : category.getItemSpecs()){
                if(item.getTotalAmount() <= item.getMinAmount()){
                    Map<Supplier, Contract> map = getSuppliers(item.getName());
                    double price = -1;
                    int supplierId = 0;
                    for(Map.Entry<Supplier,Contract> entry : map.entrySet()){
                        if(price == -1){
                            price = entry.getValue().getItemPrice(item.getName(), item.getMinAmount());
                            supplierId = entry.getKey().getId();
                        }
                        else{
                            if(price > entry.getValue().getItemPrice(item.getName())){
                                price = entry.getValue().getItemPrice(item.getName());
                                supplierId = entry.getKey().getId();
                            }
                        }
                    }
                    int orderId = OrderController.getInstance().openSingleOrder(supplierId);
                    OrderController.getInstance().addItem(orderId, item.getName(), item.getMinAmount());
                    OrderController.getInstance().submitOrder(orderId);
                }
            }
        }
    }
